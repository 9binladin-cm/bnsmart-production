-- Day Neramit phases 6-11
-- Job cost control, payments, warranty, KPI, AI evidence storage and hardening.
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format('CREATE POLICY %I ON public.%I FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id)', 'owner_all_' || t, t);
  END LOOP;
END $$;
...
-- Per-user quota guard for paid external APIs.
...
CREATE POLICY api_usage_owner_read ON public.api_usage_events FOR SELECT USING (auth.uid() = user_id);
